...
async function ForgetPass(req, res) {
    ...
    if(isValidParam.error)
        return res.status(404).send(isValidParam.error)
    ...
    if(response.length > 0 ) {
        ...
        sendEmailForgetPass(emailSender, passEmailSender, req.body.email, token_recovery)
        return res.status(200).send('Email de recuperação de senha enviado com sucesso')
    }
    else 
...
async function NewPass(req, res) {
    ...
    if(isValidTokenRecovery.length > 0) {
        ...
        bcrypt.hash(req.body.newpass, saltRounds).then(async function (hash){
            const responseUpdate = await User.update({password: hash}, {
                where: {
                    id: iduser
                }
            })
            if(responseUpdate.length > 0)
                ...
                return res.status(200).send('Erro ao alterar senha do usuário')            
        }).catch(err => {
