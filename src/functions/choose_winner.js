...
async function chooseWinnerVinte() {
  ...
  try {
    ...
    /// voltando Promise { [ user ] }

    //////////////
    const atualFragUser = arrGanhadores.map(async (winners) => {
      const fragUser = await FragUser.findAll({
        where: {
          idUser: winners.idUser,
        },
      });
      // console.log(fragUser)
      console.log(fragUser);
      return fragUser;
    });
    await Promise.all(atualFragUser);
    atualFragUser.map((element) => {
      console.log(element.Promise[0]);
    });
  } catch (err) {
